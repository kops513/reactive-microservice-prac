...
trait Metrics{
  ...
  private lazy val metricsManagerRef= requestFlow.via(metricsConnectionFlow).to(Sink.ignore).runWith(metricsSource)
  ...
  private var buffer: Vector[Metric]  = immutable.Vector.empty
