...
class Service(gateway: Gateway, repository: Repository)(implicit ec: ExecutionContext) extends Config {
  ...
  def activateCode(request: ActivateCodeRequest): Future[Either[String, ActivateCodeResponse]] = {
      ...
      codes map { t => t.length match {
        case 0 => Left("You don't have available codes")
        case _ =>
          val codeAct = t(random.nextInt(t.length))
  ...
  def login(request: LoginRequest, tokenValueOption: Option[String]): Future[Either[String, Token]] = {
    repository.useCode(request.userIdentifier, request.cardIndex, request.codeIndex, request.code) flatMap{
      case 1 =>
        tokenValueOption match {
          case None => repository.getIdentity(request.userIdentifier).flatMap{
            case Some(t) =>
              val token = gateway.requestLogin(t)
                token.map(Right(_))
            case None => Future(Left("User Identity not Found"))
          }
      ...
      case 0 => Future.successful(Left("Invalid code"))
    }
  }
  ...
  def getCodeCard(request: GetCodeCardRequest, tokenValueOption: Option[String]): Future[Either[String, GetCodeCardResponse]] = {
    tokenValueOption match {
      case Some(tokenValue) =>
        gateway.requestRelogin(tokenValue).flatMap {
          case None => Future(Left("Token expired or not found"))
          case Some(token) if repository.getIdentity(request.userIdentifier) == token.identityId =>
            repository.getNextCardIndex(request.userIdentifier).map(t => Right(GetCodeCardResponse(request.userIdentifier, generateCodeCard(t, request.userIdentifier))))

          case Some(token) => Future(Left("Token expired or not found"))
        }
